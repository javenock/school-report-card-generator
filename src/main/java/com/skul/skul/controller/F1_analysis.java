...
@RequestMapping("/f1_class_analysis")
@Controller
public class F1_analysis {

	@Autowired
	FormOneSubjectRankingService myPdfService;
	...
	@RequestMapping(value = "/one_analysis_gen_/{year}/{term}/{examtype}", method = RequestMethod.GET,produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<InputStreamResource> one_analysis_Report(@PathVariable String term, @PathVariable String year,@PathVariable String examtype ) throws FileNotFoundException, DocumentException {


        ByteArrayInputStream bis = citiesReportw(term, year, examtype);

        HttpHeaders  headers = new HttpHeaders();
        headers.add("Content-Disposition", "inline; filename=form_ONE_MARK_SHEET.pdf");
	...
	@GetMapping(value = "/one_analysis_gen_")
	public String getTecMarksEntryByStramForm(@ModelAttribute("term") String term,@ModelAttribute("year") String year, @ModelAttribute("examtype") String examtype, Model model) {
	    return "redirect:/f1_class_analysis/one_analysis_gen_/" + year +"/" + term + "/" + examtype;          
	   ...
	   public  ByteArrayInputStream citiesReportw(String term, String year, String examtype) throws DocumentException {
		...
		List<Formonebackup> rowMarks =  myPdfService.selectAll(term, year, examtype);
        ...
        try {
            ...
            PdfPCell pdfP_pos1 = new PdfPCell(new Phrase("                     --     "+ "EXAM : " +examtype +"    " +"TERM : "+term+"  "+"YEAR : "+year+ "   "+"F1"+ "   --",font4 ));
            ...
            hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table.addCell(hcell);
          for (Formonebackup mks : rowMarks) {
