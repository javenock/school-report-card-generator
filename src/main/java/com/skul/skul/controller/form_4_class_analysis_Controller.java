...
@RequestMapping("/f4_class_analysis")
@Controller
public class form_4_class_analysis_Controller {
	...
	@RequestMapping(value = "/analysis_gen_/{year}/{term}/{examtype}", method = RequestMethod.GET,produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<InputStreamResource> form_4_Report_analysis(@PathVariable String term, @PathVariable String year,@PathVariable String examtype ) throws FileNotFoundException, DocumentException {


        ByteArrayInputStream bis = form_4_analysis(term, year, examtype);

        HttpHeaders  headers = new HttpHeaders();
        headers.add("Content-Disposition", "inline; filename=form_FOUR_ANALYSIS.pdf");
	...
	@GetMapping(value = "/analysis_gen_")
	public String getTecMarksEntryByStramForm(@ModelAttribute("term") String term,@ModelAttribute("year") String year, @ModelAttribute("examtype") String examtype, Model model) {
	    return "redirect:/f4_class_analysis/analysis_gen_/" + year +"/" + term + "/" + examtype;          
	    
	}
	
	
	public  ByteArrayInputStream form_4_analysis(String term, String year, String examtype) throws DocumentException {
		
		List<Form4basebackup> rowMarks =  form4SubjectRankingService.selectAll(term, year, examtype);

		List<Subject_Analysis> sub_analy =  form4SubjectRankingService.sub_analysis(term, year, examtype);

		List<Subject_Analysis> stream_analy =  form4SubjectRankingService.stream_analysis(term, year, examtype);
        ...
        try {
            ...
            PdfPCell pdfP_pos1 = new PdfPCell(new Phrase("                     --     "+ "EXAM : " +examtype +"    " +"TERM : "+term+"  "+"YEAR : "+year+ "   "+"F4"+ "   --",font4 ));
            ...
            hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table.addCell(hcell);
          for (Form4basebackup mks : rowMarks) {
        	...
        	String eng = "--";         String kis = "--";             String math = "--";            String bio = "--";         String chem = "--";
   			...
   			String tot = "--"; String ovpoints = "--"; String ovpgrade = "--"; String crnk = "--"; String rank = "--";
   			 ...
   			 kcpemarks = mks.getKcpemarks(); subono = mks.getSubno();  
  ...
  /* eng = mks.getEng().toString();    kis = mks.getKis().toString();    math = mks.getMath().toString();   bio = mks.getBio().toString();  chem = mks.getChem().toString();
   ...
   bis = mks.getBiz().toString();    comp = mks.getComp().toString();    hom = mks.getHom().toString();   */
    ...
    hom_grade = mks.getHom_grade();
   	...
   	tot = mks.getTt().toString(); ovpoints = mks.getOv_pnts().toString(); ovpgrade = mks.getOv_pnts_grade(); crnk = mks.getCrank(); rank = mks.getRank();
                ...
                cell = new PdfPCell(new Phrase(subono,font3));
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                table.addCell(cell);
