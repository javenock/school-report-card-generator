...
@RequestMapping("/f3_class_analysis")
@Controller
public class MyPdfController {

	@Autowired
	FormThreeSubjectRankingService myPdfService;
    ...
    public ResponseEntity<InputStreamResource> citiesReport(@PathVariable String term, @PathVariable String year,@PathVariable String examtype ) throws FileNotFoundException, DocumentException {

		//List<Formthreebasebackup> rowMarks =  myPdfService.selectAll(term, year, examtype);

        ByteArrayInputStream bis = citiesReportw(term, year, examtype);

        HttpHeaders  headers = new HttpHeaders();
        headers.add("Content-Disposition", "inline; filename=citiesreport1.pdf");
	...
	public String getTecMarksEntryByStramForm(@ModelAttribute("term") String term,@ModelAttribute("year") String year, @ModelAttribute("examtype") String examtype, Model model) {
	    return "redirect:/f3_class_analysis/analysis_gen_/" + year +"/" + term + "/" + examtype;          
	    
	}
	
	
	public  ByteArrayInputStream citiesReportw(String term, String year, String examtype) throws DocumentException {
		
		List<Formthreebasebackup> rowMarks =  myPdfService.selectAll(term, year, examtype);

		List<Subject_Analysis> sub_analy =  myPdfService.sub_analysis(term, year, examtype);

		List<Subject_Analysis> stream_analy =  myPdfService.stream_analysis(term, year, examtype);
        ...
        try {
            ...
            PdfPCell pdfP_pos1 = new PdfPCell(new Phrase("                     --     "+ "EXAM : " +examtype +"    " +"TERM : "+term+"  "+"YEAR : "+year+ "   "+"F3"+ "   --",font4 ));
            ...
            hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table.addCell(hcell);
          for (Formthreebasebackup mks : rowMarks) {
