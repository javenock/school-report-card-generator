...
@Component
public class FormOneSubjectRankingDaoImpl implements FormOneSubjectRankingDao{

	@Autowired
	NamedParameterJdbcTemplate namedParameterJdbcTemplate;
	final String SQL_RANK_average = "SELECT regno,eng,RANK() OVER (ORDER BY eng DESC) AS eng_pos FROM formonemarksreportprocess";
	final String SQL_kis_average = "SELECT regno,kis,RANK() OVER (ORDER BY kis DESC) AS kis_pos FROM formonemarksreportprocess";
	final String SQL_math_average = "SELECT regno,math,RANK() OVER (ORDER BY math DESC) AS math_pos FROM formonemarksreportprocess";	
	final String SQL_bio_average = "SELECT regno,bio,RANK() OVER (ORDER BY bio DESC) AS bio_pos FROM formonemarksreportprocess";
	final String SQL_chem_average = "SELECT regno,chem,RANK() OVER (ORDER BY chem DESC) AS chem_pos FROM formonemarksreportprocess";	
	final String SQL_phy_average = "SELECT regno,phy,RANK() OVER (ORDER BY phy DESC) AS phy_pos FROM formonemarksreportprocess";	
	final String SQL_geo_average = "SELECT regno,geo,RANK() OVER (ORDER BY geo DESC) AS geo_pos FROM formonemarksreportprocess";	
	final String SQL_his_average = "SELECT regno,his,RANK() OVER (ORDER BY his DESC) AS his_pos FROM formonemarksreportprocess";	
	final String SQL_cre_average = "SELECT regno,cre,RANK() OVER (ORDER BY cre DESC) AS cre_pos FROM formonemarksreportprocess";	
	final String SQL_agric_average = "SELECT regno,agric,RANK() OVER (ORDER BY agric DESC) AS agric_pos FROM formonemarksreportprocess";	
	final String SQL_biz_average = "SELECT regno,biz,RANK() OVER (ORDER BY biz DESC) AS biz_pos FROM formonemarksreportprocess";	
	final String SQL_comp_average = "SELECT regno,comp,RANK() OVER (ORDER BY comp DESC) AS comp_pos FROM formonemarksreportprocess";	
	final String SQL_hom_average = "SELECT regno,hom,RANK() OVER (ORDER BY hom DESC) AS hom_pos FROM formonemarksreportprocess";	
	
	final  String SQL_SELECT_TERM_ONE = "SELECT * from form_one_marks_back_up where regno = :regno AND term = '1' ";
	 
	 final String SQL_SELECT_TERM_TWO = "SELECT * from form_one_marks_back_up where regno = :regno AND term = '2' ";
	 
	 final String SQL_SELECT_TERM_THREE = "SELECT * from form_one_marks_back_up where regno = :regno AND term = '3' ";
	 
     final  String SQL_SELECT_EXAM_ONE = "SELECT * from formonebackup where regno = :regno AND term = :term AND year = :year AND examtype = 'EXAM1' ";
	 
	 final String SQL_SELECT_EXAM_TWO = "SELECT * from formonebackup where regno = :regno AND term = :term AND year = :year AND examtype = 'EXAM2' ";
	 
	 final String SQL_SELECT_EXAM_THREE = "SELECT * from formonebackup where regno = :regno AND term = :term AND year = :year AND examtype = 'EXAM3' ";
	 
	 final  String SQL_SELECT_EXAM_ = "SELECT * from form_one_marks_back_up where regno = :regno AND term = :term AND year = :year";
	 
	 final String SQL_DEV_ = "SELECT id,regno,COALESCE(tt,0) AS tt,tt - lag(tt) OVER (ORDER BY id) as deviation FROM form_one_marks_back_up WHERE regno = :regno ORDER BY id DESC LIMIT 1";
		
	 final String SQL_SELECT_MARKCHIT = "SELECT student.regno,student.stream,student.fname,student.subno,student.kcpemarks,student.kcpemean,student.kcpegrade,formonebackup.* FROM student, formonebackup WHERE student.regno = formonebackup.regno AND formonebackup.term = :term AND formonebackup.year = :year AND formonebackup.examtype = :examtype ORDER BY tt DESC";
	 ...
	 final String SQL_SUB_ANALYSIS = "SELECT SUM(CASE WHEN eng_grade  = 'A' THEN 1 ELSE 0 END) AS eng_aplain, SUM(CASE WHEN eng_grade  = 'A-' THEN 1 ELSE 0 END) AS eng_aminus,SUM(CASE WHEN eng_grade  = 'B+' THEN 1 ELSE 0 END) AS eng_bplus,SUM(CASE WHEN eng_grade  = 'B' THEN 1 ELSE 0 END) AS eng_bplain,SUM(CASE WHEN eng_grade  = 'B-' THEN 1 ELSE 0 END) AS eng_bminus,SUM(CASE WHEN eng_grade  = 'C+' THEN 1 ELSE 0 END) AS eng_cplus,SUM(CASE WHEN eng_grade  = 'C' THEN 1 ELSE 0 END) AS eng_cplain,SUM( CASE WHEN eng_grade  = 'C-' THEN 1 ELSE 0 END) AS eng_cminus,SUM( CASE WHEN eng_grade  = 'D+' THEN 1 ELSE 0 END) AS eng_dplus,SUM( CASE WHEN eng_grade  = 'D' THEN 1 ELSE 0 END) AS eng_dplain,SUM( CASE WHEN eng_grade  = 'D-' THEN 1 ELSE 0 END) AS eng_dminus,SUM( CASE WHEN eng_grade  = 'E' THEN 1 ELSE 0 END) AS eng_eplain,(SUM( CASE WHEN eng_grade  = 'A' THEN 1 ELSE 0 END) + SUM( CASE WHEN eng_grade  = 'A-' THEN 1 ELSE 0 END) +SUM( CASE WHEN eng_grade  = 'B+' THEN 1 ELSE 0 END) +	SUM( CASE WHEN eng_grade  = 'B' THEN 1 ELSE 0 END) +SUM( CASE WHEN eng_grade  = 'B-' THEN 1 ELSE 0 END) +SUM( CASE WHEN eng_grade  = 'C+' THEN 1 ELSE 0 END) + SUM( CASE WHEN eng_grade  = 'C' THEN 1 ELSE 0 END) +SUM( CASE WHEN eng_grade  = 'C-' THEN 1 ELSE 0 END) +SUM( CASE WHEN eng_grade  = 'D+' THEN 1 ELSE 0 END) +SUM(CASE WHEN eng_grade  = 'D' THEN 1 ELSE 0 END) +SUM( CASE WHEN eng_grade  = 'D-' THEN 1 ELSE 0 END) +SUM( CASE WHEN eng_grade  = 'E' THEN 1 ELSE 0 END ))AS eng_Entry,SUM(eng) AS eng_av,\r\n" + 
				...
				"FROM formonebackup WHERE term = :term AND year = :year AND examtype = :examtype ";
	 final String SQL_STREAM_ANALYSIS = "SELECT stream,SUM(CASE WHEN ov_grade  = 'A' THEN 1 ELSE 0 END) AS aplain, SUM(CASE WHEN ov_grade  = 'A-' THEN 1 ELSE 0 END) AS aminus,SUM(CASE WHEN ov_grade  = 'B+' THEN 1 ELSE 0 END) AS bplus,SUM(CASE WHEN ov_grade  = 'B' THEN 1 ELSE 0 END) AS bplain,SUM(CASE WHEN ov_grade  = 'B-' THEN 1 ELSE 0 END) AS bminus,SUM(CASE WHEN ov_grade  = 'C+' THEN 1 ELSE 0 END) AS cplus,SUM(CASE WHEN ov_grade  = 'C' THEN 1 ELSE 0 END) AS cplain,SUM( CASE WHEN ov_grade  = 'C-' THEN 1 ELSE 0 END) AS cminus,SUM( CASE WHEN ov_grade  = 'D+' THEN 1 ELSE 0 END) AS dplus,SUM( CASE WHEN ov_grade  = 'D' THEN 1 ELSE 0 END) AS dplain,SUM( CASE WHEN ov_grade  = 'D-' THEN 1 ELSE 0 END) AS dminus,SUM( CASE WHEN ov_grade  = 'E' THEN 1 ELSE 0 END) AS eplain,(SUM( CASE WHEN ov_grade  = 'A' THEN 1 ELSE 0 END) + SUM( CASE WHEN ov_grade  = 'A-' THEN 1 ELSE 0 END) +SUM( CASE WHEN ov_grade  = 'B+' THEN 1 ELSE 0 END) +	SUM( CASE WHEN ov_grade  = 'B' THEN 1 ELSE 0 END) +SUM( CASE WHEN ov_grade  = 'B-' THEN 1 ELSE 0 END) +SUM( CASE WHEN ov_grade  = 'C+' THEN 1 ELSE 0 END) + SUM( CASE WHEN ov_grade  = 'C' THEN 1 ELSE 0 END) +SUM( CASE WHEN ov_grade  = 'C-' THEN 1 ELSE 0 END) +SUM( CASE WHEN ov_grade  = 'D+' THEN 1 ELSE 0 END) +SUM(CASE WHEN ov_grade  = 'D' THEN 1 ELSE 0 END) +SUM( CASE WHEN ov_grade  = 'D-' THEN 1 ELSE 0 END) +SUM( CASE WHEN ov_grade  = 'E' THEN 1 ELSE 0 END ))AS entry,(SUM(COALESCE(CAST(tt AS DOUBLE PRECISION),0))) AS  tt_marks FROM formonebackup WHERE term = :term AND year = :year AND examtype = :examtype GROUP BY stream";
	...
	public int[] sub_rank_average(List<FormOneMarks_Report_Process> subrankAvg) {
        ...
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,eng_pos = :eng_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}	
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_ave() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_RANK_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process eng_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_kis(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());  
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,kis_pos = :kis_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_kis() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_kis_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process kis_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_math(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,math_pos = :math_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_math() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_math_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process math_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_bio(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,bio_pos = :bio_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_bio() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_bio_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process bio_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_chem(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,chem_pos = :chem_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_chem() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_chem_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process chem_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_phy(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,phy_pos = :phy_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_phy() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_phy_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process phy_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_geo(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,geo_pos = :geo_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_geo() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_geo_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process geo_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_his(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,his_pos = :his_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_his() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_his_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process his_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_cre(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,cre_pos = :cre_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_cre() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_cre_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process cre_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_agric(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,agric_pos = :agric_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_agric() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_agric_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process agric_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_biz(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,biz_pos = :biz_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_biz() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_biz_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process biz_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_comp(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,comp_pos = :comp_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_comp() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_comp_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process comp_rank = new FormOneMarks_Report_Process();					
	...
	public int[] sub_rank_hom(List<FormOneMarks_Report_Process> subrankAvg) {
		SqlParameterSource[] batch = SqlParameterSourceUtils.createBatch(subrankAvg.toArray());
        int[] crankCounts = namedParameterJdbcTemplate.batchUpdate(
        		"update formonemarksreportprocess set regno = :regno,hom_pos = :hom_pos where regno = :regno",batch);        		
        	      return crankCounts;
	}
	@Override
	public List<FormOneMarks_Report_Process> sub_rank_hom() {
		List<FormOneMarks_Report_Process> list = namedParameterJdbcTemplate.query(SQL_hom_average, new RowMapper<FormOneMarks_Report_Process>() {

			@Override
			public FormOneMarks_Report_Process mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarks_Report_Process hom_rank = new FormOneMarks_Report_Process();					
	...
	public List<FormOneMarksBackUp> selectAll_TermOne(Long adno) {
		List<FormOneMarksBackUp> list = namedParameterJdbcTemplate.query(SQL_SELECT_TERM_ONE, new MapSqlParameterSource().addValue("regno", adno), new RowMapper<FormOneMarksBackUp>() {
			
				@Override
				public FormOneMarksBackUp mapRow(ResultSet rs, int rowNum) throws SQLException {
					FormOneMarksBackUp s_all = new FormOneMarksBackUp();
	...
	public List<FormOneMarksBackUp> selectAll_TermTwo(Long adno) {
		List<FormOneMarksBackUp> list = namedParameterJdbcTemplate.query(SQL_SELECT_TERM_TWO, new MapSqlParameterSource().addValue("regno", adno), new RowMapper<FormOneMarksBackUp>() {
	...
	public List<FormOneMarksBackUp> selectAll_TermThree(Long adno) {
		List<FormOneMarksBackUp> list = namedParameterJdbcTemplate.query(SQL_SELECT_TERM_THREE, new MapSqlParameterSource().addValue("regno", adno), new RowMapper<FormOneMarksBackUp>() {
	...
	public  List<FormOneMarks> selectAll_ExamOne(Long adno, String term, String year) {
		List<FormOneMarks> list = namedParameterJdbcTemplate.query(SQL_SELECT_EXAM_ONE, new MapSqlParameterSource().addValue("regno", adno).addValue("term", term).addValue("year", year), new RowMapper<FormOneMarks>() {
	...
	public List<FormOneMarks> selectAll_ExamTwo(Long adno, String term, String year) {
		...
		List<FormOneMarks> list = namedParameterJdbcTemplate.query(SQL_SELECT_EXAM_TWO, new MapSqlParameterSource().addValue("regno", adno).addValue("term", term).addValue("year", year), new RowMapper<FormOneMarks>() {
	...
	public List<FormOneMarks> selectAll_ExamThree(Long adno, String term,String year) {
		...
		List<FormOneMarks> list = namedParameterJdbcTemplate.query(SQL_SELECT_EXAM_THREE,new MapSqlParameterSource().addValue("regno", adno).addValue("term", term).addValue("year", year), new RowMapper<FormOneMarks>() {
	...
	public  List<FormOneMarksBackUp> selectAll_Exam(Long adno, String term, String year) {
		List<FormOneMarksBackUp> list = namedParameterJdbcTemplate.query(SQL_SELECT_EXAM_, new MapSqlParameterSource().addValue("regno", adno).addValue("term", term).addValue("year", year), new RowMapper<FormOneMarksBackUp>() {
	...
	public List<FormOneMarksBackUp> student_mean_dev(long regno) {
		List<FormOneMarksBackUp> list = namedParameterJdbcTemplate.query(SQL_DEV_,new MapSqlParameterSource().addValue("regno", regno), new RowMapper<FormOneMarksBackUp>() {
			@Override
			public FormOneMarksBackUp mapRow(ResultSet rs, int rowNum) throws SQLException {
				FormOneMarksBackUp s_all = new FormOneMarksBackUp();
				s_all.setRegno(rs.getLong("regno"));
	...
	List<Formonebackup> list = namedParameterJdbcTemplate.query(SQL_SELECT_MARKCHIT, new MapSqlParameterSource().addValue("term", term).addValue("year", year).addValue("examtype", examtype), new RowMapper<Formonebackup>() {
			
			@Override
			public Formonebackup mapRow(ResultSet rs, int rowNum) throws SQLException {
				Formonebackup s_all = new Formonebackup();
				s_all.setRegno(rs.getLong("regno"));
